...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        return grad_output * result * (result.f.neg_map(result) + 1.0)
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        permutation = tuple(int(order[i]) for i in range(order.size))
        ctx.save_for_backward(permutation)
        return a._new(a._tensor.permute(*permutation))

    @staticmethod
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        (permutation,) = ctx.saved_values
        inverse = tuple(permutation.index(i) for i in range(len(permutation)))
