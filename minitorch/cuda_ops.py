...
def _sum_practice(out: Storage, a: Storage, size: int) -> None:
    ...
    while stride > 0:
        ...
        cuda.syncthreads()
        stride //= 2
    if pos == 0 and cuda.blockIdx.x < len(out):
...
def tensor_reduce(
    ...
    def _reduce(
        ...
        reduction_index = out_index[reduce_dim] * BLOCK_DIM + pos
        if reduction_index < a_shape[reduce_dim]:
            cache[pos] = a_storage[
                a_position
                + (reduction_index - out_index[reduce_dim]) * a_strides[reduce_dim]
            ]
        else:
...
def _mm_practice(out: Storage, a: Storage, b: Storage, size: int) -> None:
    ...
    * Only write to global memory once per kernel.
    ...
    if row < size and column < size:
        value = 0.0
        for k in range(size):
            value += a_shared[row, k] * b_shared[k, column]
        out[row * size + column] = value
...
def _tensor_matrix_multiply(
    ...
    * Only write to global memory once per kernel.
    ...
    for tile in range((a_shape[2] + BLOCK_DIM - 1) // BLOCK_DIM):
        ...
        if i < a_shape[1] and a_column < a_shape[2]:
            a_shared[pi, pj] = a_storage[
                batch * a_batch_stride + i * a_strides[1] + a_column * a_strides[2]
            ]
        else:
            a_shared[pi, pj] = 0.0
        if b_row < b_shape[1] and j < b_shape[2]:
            b_shared[pi, pj] = b_storage[
                batch * b_batch_stride + b_row * b_strides[1] + j * b_strides[2]
            ]
        else:
            b_shared[pi, pj] = 0.0
        cuda.syncthreads()
